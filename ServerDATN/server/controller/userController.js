...
module.exports.getSchedules = async (req, res) => {
    ...
    await Schedule.find({status:new RegExp(status, 'i')}).populate('idUser').populate('vehicle').then((schedule) => {
        res.render('schedule/schedules', {layout: 'temp/index', data: schedule,  title: "Danh sách lịch đặt"})
    }, (err) => {
        res.render('error/404', {layout: 'temp/index', title: "Có lỗi xảy ra !", err: true, message: err})
    }).then((err) => {
        res.render('error/404', {layout: 'temp/index', title: "Có lỗi xảy ra !", err: true, message: err})
...
module.exports.confirm = async (req, res) => {
    ...
    if (perform == "Confirm") {
        ...
        if (!user) {
            res.render('error/404', {layout: 'temp/index', title: "Có lỗi xảy ra !", err: true, message: 'Không nhận dạng được người dùng. Vui lòng đăng nhập lại!'})
            return
        }
        let schedule = await Schedule.findById(req.query.id);
        if (!schedule) {
            res.render('error/404', {layout: 'temp/index', title: "Có lỗi xảy ra !", err: true, message: 'Không tìm thấy lịch đặt. Vui lòng thử lại!'})
            return
        }
        if (schedule.status != 'Pending') {
            res.render('error/404', {layout: 'temp/index', title: "Có lỗi xảy ra !", err: true, message: 'Lịch này đã được xác nhận hoặc đã hoàn thành'})
            return
        }
